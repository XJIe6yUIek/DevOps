...
class InputConect:
    def make(self):
        ...
        vac_name = input('Введите название профессии: ').split(' ')
    ...
    def printing_data_for_table(dataset, vac_name):
        ...
        for vacancie in dic_vacancies:
            ...
            years_count_dic[y] += 1
            if any([i in vacancie.name for i in vac_name]):
        ...
        x_line = tuple(years_salary_dic)
        y_line = tuple(years_salary_dic[i] for i in x_line)
        plt.plot(x_line, y_line)
        plt.savefig('years_salary.png')
        plt.clf()

        x_line = tuple(years_count_dic)
        y_line = tuple(years_count_dic[i] for i in x_line)
        plt.plot(x_line, y_line)
        plt.savefig('years_count.png')
        plt.clf()

        x_line = tuple(years_salary_vac_dic)
        y_line = tuple(years_salary_vac_dic[i] for i in x_line)
        plt.plot(x_line, y_line)
        plt.savefig('years_salary_vac.png')
        plt.clf()

        x_line = tuple(years_count_vac_dic)
        y_line = tuple(years_count_vac_dic[i] for i in x_line)
        plt.plot(x_line, y_line)
        plt.savefig('years_count_vac.png')
        plt.clf()

        x_line = tuple(area_salary_dic)
        y_line = tuple(area_salary_dic[i] for i in x_line)
        x_line = tuple(x_line[i][0:3] + '.' for i in range(0, len(x_line)))
        plt.plot(x_line, y_line)
        plt.savefig('area_salary.png')
        plt.clf()

        x_line = tuple(area_count_dic)
        y_line = tuple(area_count_dic[i] for i in x_line)
        x_line = tuple(x_line[i][0:3] + '.' for i in range(0, len(x_line)))
        plt.plot(x_line, y_line)
        plt.savefig('area_count.png')
        plt.clf()
